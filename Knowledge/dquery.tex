...
\begin{algorithm}[!htbp]
	...
	\proc{FedSparql($qp, sp, \mathcal{H}$)}{
		\# Query-path: $qp=(p_1,p_2,\dots,p_n)$ \\
		\# Schema-path: $sp=(l_1, l_2, \dots, l_n)$ \\
		\# Source-Index hierarchy: $\mathcal{H} = (SP_n, SP_{n-1}, \dots, SP_1)$ \\
		result $\leftarrow \emptyset$ \;
		\For {all sources $s_i$ in $SP_n$}{
			\# answer the query and add the results to the result set \\
			result $\leftarrow$ result $\cup$ {\bf answer}($s_i, qp$)\;
		} % EndFor
		\If{$n \geq 2$ }{
			\For {all $i$ in $1, \dots, n-1$} {
				\# split the query/schema-path into two pieces and answer them \\
				...
				\# join the part answers, and add joined answers to the result set \\
